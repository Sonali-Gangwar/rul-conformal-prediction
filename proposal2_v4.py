"""
proposal2_v4.py — Proposal 2 FINAL: CNN+GRU DegradationFM
Fixes: 60 epoch fine-tuning + multiple calibration cutpoints
"""
...
CMAPSS_COLS = ["engine","cycle","op1","op2","op3"]+[f"s{i}" for i in range(1,22)]
...
def multi_rand_cut(X, y, engines, cycles, seq_len, n_cuts=5):
    """Multiple random cutpoints per engine for better calibration."""
    all_seqs, all_y = [], []
    for seed_i in range(n_cuts):
        cs, cy = rand_cut(X, y, engines, cycles, seq_len, seed=seed_i)
        all_seqs.append(cs); all_y.append(cy)
    return np.vstack(all_seqs), np.concatenate(all_y)
...
class DegradationFM(nn.Module):
    ...
    def encode(self, x):
        x_t=x.transpose(1,2)
        c=self.cnn(x_t).transpose(1,2)
        _,h=self.gru(c)
        return h[-1]

    def forward_pretrain(self, x, masked_x):
        emb=self.encode(masked_x)
        return self.pretrain_head(emb), x[:,-1,:]

    def add_rul_head(self):
        self.rul_head=nn.Sequential(
            nn.Linear(self.d_model,64), nn.ReLU(),
...
def main():
    print("="*62)
    print("  PROPOSAL 2 v4: CNN+GRU DegradationFM (Final)")
    print("="*62)
    ...
    print(f"\n  Total: {len(pretrain_seqs):,} seqs")

    # STEP 2: Pre-train
    print("\n[STEP 2] Pre-training DegradationFM ...")
    ...
    torch.save(model.state_dict(),r"D:\rul_project\degradation_fm_v4.pt")
    print("  Saved: degradation_fm_v4.pt")

    # STEP 3: Fine-tune on C-MAPSS
    print("\n[STEP 3] Fine-tuning (60 epochs + multi-cutpoint calibration) ...")
    results=[]
    for fd in ["FD001","FD002","FD003","FD004"]:
        print(f"\n  --- {fd} ---")
        X,engines,cycles,sc,fc,rul=load_cmapss(fd,"train")
        ...
        # FIX: multiple random cutpoints for better calibration
        cal_seqs,cal_y=multi_rand_cut(X[mca],yca,eca,cca,SEQ_LEN,n_cuts=5)
    ...
    for r in results:
        ...
        print(f"  {r['fd']:<6}{r['MAE']:>9}{p1[r['fd']]:>9}"
              f"{lstm[r['fd']]:>9}{r['cov']:>8}{r['late']:>8}  "
              f"{r['honest']} {'<- P2 beats P1' if b else ''}")
    print(f"{'='*70}")
    print("\n  KEY: Does domain-specific pre-training on degradation data")
    print("  beat Chronos-2 (general time-series foundation model)?")
